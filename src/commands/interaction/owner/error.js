...
import ms from 'ms';
import interactionPaginator from "../../../utils/design/interactionPaginator.js";
	...
	subcommands: [{
			...
			if (!error) {
				...
				let embed = new Builders.EmbedBuilder()
					...
					.addFields([{
						...
						name: "Time", value: `<t:${Math.floor(error.time.getTime() / 1000)}:F>`
					}, {
		...
		async execute(interaction, client, args, Discord) {
			...
			if (interaction.options.getString("status")) {
				errors = errors.filter(error => error.status === interaction.options.getString("status"));
			}
			if (errors.length === 0) {
				let embed = new Builders.EmbedBuilder()
					.setColor(global.config.colors.success)
