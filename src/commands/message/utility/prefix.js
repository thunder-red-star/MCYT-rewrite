...
export default {
	...
	execute: async function(message, client, args, Discord) {
		// Get prefix
		let prefix = await global.database.guild.get.by({_id: message.guild.id})
		prefix = prefix.prefix;
		if (!prefix) prefix = global.config.defaultPrefix;
		// Check if prefix is the same
		if (args.prefix === null || args.prefix === undefined) {
			// Reset prefix to default
			await global.database.guild.update.by(message.guild.id, {prefix: global.config.defaultPrefix});
			// Send message
		...
		// Set prefix
		await global.database.guild.update.by(message.guild.id, {prefix: args.prefix});
		// Reply
		const embed = new Builders.EmbedBuilder()
			.setColor(global.config.colors.success)
