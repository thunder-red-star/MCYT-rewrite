...
export default async function commandLoader(client, Discord) {
	client.categories = JSON.parse(fs.readFileSync(path.join(__dirname, 'categories.json'), 'utf8'));
	client.messageCommands = new Discord.Collection();
	client.messageCommandAliases = new Discord.Collection();
	...
	for (const category of client.categories) {
		...
		for (const file of commandFiles) {
			let command = await import("./message/" + category + "/" + file);
	...
	for (const category of client.categories) {
		...
		for (const file of commandFiles) {
			let command = await import("./interaction/" + category + "/" + file);
			command = command.default;
